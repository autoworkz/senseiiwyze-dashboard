...
export const invitations = pgTable("invitations", {
	...
	pgPolicy("invitations_create_self", { as: "permissive", for: "insert", to: ["authenticated"], withCheck: sql`(is_set('enable_team_accounts'::text) AND has_permission(( SELECT auth.uid() AS uid), account_id, 'invites.manage'::app_permissions) AND (has_more_elevated_role(( SELECT auth.uid() AS uid), account_id, role) OR has_same_role_hierarchy_level(( SELECT auth.uid() AS uid), account_id, role)))` }),
...
export const orders = pgTable("orders", {
	...
	pgPolicy("orders_read_self", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(((account_id = ( SELECT auth.uid() AS uid)) AND is_set('enable_account_billing'::text)) OR (has_role_on_account(account_id) AND is_set('enable_team_account_billing'::text)))` }),
	pgPolicy("restrict_mfa_orders", { as: "restrictive", for: "all", to: ["authenticated"] }),
...
export const categories = pgTable("categories", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "bigint" }).primaryKey().generatedByDefaultAsIdentity({ name: "categories_id_seq", startWith: 1, increment: 1, minValue: 1, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	name: varchar(),
});
...
export const goals = pgTable("goals", {
	id: text().default(schemaUtils.genRandomUuid).primaryKey().notNull(),
	name: text().notNull(),
...
export const chatMessages = pgTable("chat_messages", {
	...
	pgPolicy("delete_chat_messages", { as: "permissive", for: "delete", to: ["authenticated"], using: sql`has_role_on_account(account_id)` }),
	pgPolicy("select_chat_messages", { as: "permissive", for: "select", to: ["authenticated"] }),
]);
...
export const orderItems = pgTable("order_items", {
	...
	pgPolicy("order_items_read_self", {
		as: "permissive", for: "select", to: ["authenticated"], using: sql`(EXISTS ( SELECT 1
   FROM orders
  WHERE ((orders.id = order_items.order_id) AND ((orders.account_id = ( SELECT auth.uid() AS uid)) OR has_role_on_account(orders.account_id)))))` }),
	pgPolicy("restrict_mfa_order_items", { as: "restrictive", for: "all", to: ["authenticated"] }),
...
export const lcaInvitations = pgTable("lca_invitations", {
	...
	accountRole: accountRole("account_role").notNull(),
	accountId: uuid("account_id").notNull(),
	token: text().default(sql`uuid_generate_v4()`).notNull(),
...
export const rolePermissions = pgTable("role_permissions", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "bigint" }).primaryKey().generatedByDefaultAsIdentity({ name: "role_permissions_id_seq", startWith: 1, increment: 1, minValue: 1, cache: 1 }),
	role: varchar({ length: 50 }).notNull(),
...
export const tasks = pgTable("tasks", {
	...
	pgPolicy("delete_tasks", { as: "permissive", for: "delete", to: ["public"], using: sql`((account_id = ( SELECT auth.uid() AS uid)) OR has_permission(( SELECT auth.uid() AS uid), account_id, 'tasks.delete'::app_permissions))` }),
	pgPolicy("insert_tasks", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("select_tasks", { as: "permissive", for: "select", to: ["authenticated"] }),
	pgPolicy("update_tasks", { as: "permissive", for: "update", to: ["public"] }),
]);

export const vendorRatings = pgTable("vendor_ratings", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "bigint" }).primaryKey().generatedByDefaultAsIdentity({ name: "vendor_ratings_id_seq", startWith: 1, increment: 1, minValue: 1, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow(),
...
export const notifications = pgTable("notifications", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "bigint" }).primaryKey().generatedAlwaysAsIdentity({ name: "notifications_id_seq", startWith: 1, increment: 1, minValue: 1, cache: 1 }),
	accountId: uuid("account_id").notNull(),
