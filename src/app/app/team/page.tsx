...
export default function TeamManagementPage() {
  ...
  useEffect(() => {
    ...
    const mockStats: TeamStats = {
      ...
      activeMembers: mockTeamMembers.filter(
        (m) => m.status === 'active' || m.status === 'excelling'
      ).length,
      atRiskMembers: mockTeamMembers.filter(
        (m) => m.status === 'at-risk' || m.status === 'inactive'
      ).length,
      avgReadinessScore: Math.round(
        mockTeamMembers.reduce((sum, m) => sum + m.readinessScore, 0) / mockTeamMembers.length
      ),
      avgProgressPercentage: Math.round(
        mockTeamMembers.reduce((sum, m) => sum + m.progressPercentage, 0) / mockTeamMembers.length
      ),
          ...
          <InteractiveCard effect="lift">
            ...
            <InteractiveCardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {Math.round((teamStats.activeMembers / teamStats.totalMembers) * 100)}% engagement
                rate
              </p>
            </InteractiveCardContent>
          </InteractiveCard>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Earned Coaching Excellence Badge
                      </div>
                    </div>
        ...
        <TabsContent value="members" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage your team members and track their progress</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium">
                      {
                        teamMembers.filter((m) => m.readinessScore >= 60 && m.readinessScore < 80)
                          .length
                      }{' '}
                      members
                    </span>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium">
                      {
                        teamMembers.filter((m) => m.readinessScore >= 40 && m.readinessScore < 60)
                          .length
                      }{' '}
                      members
                    </span>
                  ...
                  {['Engineering', 'Product', 'Marketing', 'Learning & Development'].map((dept) => {
                    ...
                    const avgScore =
                      deptMembers.length > 0
                        ? Math.round(
                            deptMembers.reduce((sum, m) => sum + m.readinessScore, 0) /
                              deptMembers.length
                          )
                        : 0
                  ...
                  .map((coach) => {
                    ...
                    const avgScore =
                      assignedLearners.length > 0
                        ? Math.round(
                            assignedLearners.reduce((sum, m) => sum + m.readinessScore, 0) /
