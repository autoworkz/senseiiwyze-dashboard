...
export const taskstate = pgEnum("taskstate", [
  "UNTOUCHED",
  "ONGOING",
  "COMPLETED",
]);
...
export const answersIdSeq1 = pgSequence("answers_id_seq1", {
  startWith: "1",
  increment: "1",
  minValue: "1",
  maxValue: "2147483647",
...
export const profiles = pgTable(
  ...
  (table) => [
    uniqueIndex("profiles_email_key").using(
      "btree",
...
export const answers = pgTable(
  ...
  (table) => [
    ...
    unique("idx_unique_answers_for_eval").on(
      table.questionId,
      table.evaluationId,
    ),
  ],
...
export const profilesCopy = pgTable(
  "profiles_copy",
  {
    email: text().notNull(),
    name: text(),
  ...
  (table) => [
    uniqueIndex("profiles_copy_email_idx").using(
      "btree",
      table.email.asc().nullsLast().op("text_ops"),
    ),
    foreignKey({
...
export const lcaInvitations = pgTable(
  "lca_invitations",
  {
    id: uuid()
      .default(sql`uuid_generate_v4()`)
...
export const gameInfo = pgTable(
  "game_info",
  {
    ...
    avgTimePerLevel: numeric("avg_time_per_level"),
  },
  (table) => [
    foreignKey({
...
export const account = pgTable(
  ...
  (table) => [
    index("account_provider_id_account_id_idx").using(
      "btree",
...
export const verification = pgTable(
  ...
  (table) => [
    index("verification_identifier_value_idx").using(
      "btree",
...
export const questions = pgTable(
  ...
  (table) => [
    uniqueIndex("questions_feat_label_key").using(
      "btree",
      table.featLabel.asc().nullsLast().op("text_ops"),
    ),
    uniqueIndex("questions_question_key").using(
      "btree",
...
export const gameTasks = pgTable(
  "game_tasks",
  {
    id: uuid()
      .default(sql`uuid_generate_v4()`)
...
export const visionLog = pgTable(
  "vision_log",
  {
    id: text().primaryKey().notNull(),
    visionId: text("vision_id").notNull(),
...
export const workplaces = pgTable(
  "workplaces",
  {
    ...
    id: uuid().defaultRandom().primaryKey().notNull(),
    type: text(),
    enabledAssessments: uuid("enabled_assessments")
      .array()
...
export const config = pgTable(
  "config",
  {
    ...
    enableAccountBilling: boolean("enable_account_billing")
      .default(true)
      .notNull(),
...
export const invitations = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("invitations_read_self", {
      ...
      for: "select",
      to: ["authenticated"],
      using: sql`has_role_on_account(account_id)`,
    }),
...
export const billingCustomers = pgTable(
  ...
  (table) => [
    index("ix_billing_customers_account_id").using(
      "btree",
      table.accountId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
    ...
    unique("billing_customers_account_id_customer_id_provider_key").on(
      table.accountId,
      table.provider,
      table.customerId,
    ),
    pgPolicy("billing_customers_read_self", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
...
export const subscriptions = pgTable(
  ...
  (table) => [
    index("ix_subscriptions_account_id").using(
      "btree",
      table.accountId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
    ...
    pgPolicy("restrict_mfa_subscriptions", {
      as: "restrictive",
      for: "all",
      to: ["authenticated"],
      using: sql`is_mfa_compliant()`,
    }),
    pgPolicy("super_admins_access_subscriptions", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
    pgPolicy("subscriptions_read_self", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
...
export const subscriptionItems = pgTable(
  ...
  (table) => [
    index("ix_subscription_items_subscription_id").using(
      "btree",
    ...
    unique("subscription_items_subscription_id_product_id_variant_id_key").on(
      table.subscriptionId,
      table.productId,
      table.variantId,
    ),
    pgPolicy("restrict_mfa_subscription_items", {
      as: "restrictive",
      for: "all",
      to: ["authenticated"],
      using: sql`is_mfa_compliant()`,
    }),
    pgPolicy("super_admins_access_subscription_items", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
    pgPolicy("subscription_items_read_self", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
...
export const orders = pgTable(
  ...
  (table) => [
    index("ix_orders_account_id").using(
      "btree",
      table.accountId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
    ...
    pgPolicy("restrict_mfa_orders", {
      as: "restrictive",
      for: "all",
      to: ["authenticated"],
      using: sql`is_mfa_compliant()`,
    }),
    pgPolicy("super_admins_access_orders", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
    pgPolicy("orders_read_self", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
...
export const orderItems = pgTable(
  ...
  (table) => [
    index("ix_order_items_order_id").using(
      "btree",
    ...
    unique("order_items_order_id_product_id_variant_id_key").on(
      table.orderId,
      table.productId,
      table.variantId,
    ),
    pgPolicy("restrict_mfa_order_items", {
      as: "restrictive",
      for: "all",
      to: ["authenticated"],
      using: sql`is_mfa_compliant()`,
    }),
    pgPolicy("super_admins_access_order_items", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
    pgPolicy("order_items_read_self", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
...
export const notifications = pgTable(
  ...
  (table) => [
    index("idx_notifications_account_dismissed").using(
      ...
      table.dismissed.asc().nullsLast().op("uuid_ops"),
...
export const chats = pgTable(
  ...
  (table) => [
    index("ix_chats_account_id").using(
      "btree",
      table.accountId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
    ...
    pgPolicy("delete_chats", {
      as: "permissive",
      for: "delete",
      to: ["authenticated"],
      using: sql`has_role_on_account(account_id)`,
    }),
    pgPolicy("select_chats", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
    pgPolicy("insert_chats", {
      as: "permissive",
      for: "insert",
      to: ["authenticated"],
    }),
    pgPolicy("update_chats", {
      as: "permissive",
      for: "update",
      to: ["authenticated"],
    }),
...
export const tasks = pgTable(
  ...
  (table) => [
    index("ix_tasks_account_id").using(
      "btree",
      table.accountId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
    ...
    pgPolicy("select_tasks", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
      using: sql`((account_id = ( SELECT auth.uid() AS uid)) OR has_role_on_account(account_id))`,
    }),
    pgPolicy("insert_tasks", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
    pgPolicy("update_tasks", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("delete_tasks", {
      as: "permissive",
      for: "delete",
      to: ["public"],
    }),
...
export const chatMessages = pgTable(
  ...
  (table) => [
    index("ix_chat_messages_chat_id").using(
      "btree",
      table.chatId.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
      columns: [table.accountId],
      foreignColumns: [accounts.id],
    ...
    pgPolicy("select_chat_messages", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
      using: sql`has_role_on_account(account_id)`,
    }),
    pgPolicy("delete_chat_messages", {
      as: "permissive",
      for: "delete",
      to: ["authenticated"],
    }),
...
export const personalityExams = pgTable(
  "personality_exams",
  {
    id: serial().primaryKey().notNull(),
    userId: uuid("user_id"),
...
export const userSkillDetails = pgTable(
  "user_skill_details",
  {
    id: serial().primaryKey().notNull(),
...
export const examTraits = pgTable(
  "exam_traits",
  {
    id: serial().primaryKey().notNull(),
...
export const examStrengths = pgTable(
  "exam_strengths",
  {
    id: serial().primaryKey().notNull(),
    examId: integer("exam_id"),
...
export const examGrowthAreas = pgTable(
  "exam_growth_areas",
  {
    id: serial().primaryKey().notNull(),
    examId: integer("exam_id"),
...
export const programSkillRequirements = pgTable(
  ...
  (table) => [
    ...
    unique("program_skill_requirements_assessment_id_skill_key_key").on(
      table.assessmentId,
      table.skillKey,
    ),
  ],
...
export const userAccountWorkspace = pgView("user_account_workspace", {
  ...
  .as(
    sql`SELECT accounts.id, accounts.name, accounts.picture_url, ( SELECT subscriptions.status FROM subscriptions WHERE subscriptions.account_id = accounts.id LIMIT 1) AS subscription_status FROM accounts WHERE accounts.primary_owner_user_id = (( SELECT auth.uid() AS uid)) AND accounts.is_personal_account = true LIMIT 1`,
  );
