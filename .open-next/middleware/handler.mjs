...
var IgnorableError, RecoverableError;
...
// node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/utils/fetch.js
function customFetchClient(client) {
...
function parseNumberFromEnv(envValue) {
...
// node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/overrides/tagCache/constants.js
var MAX_DYNAMO_BATCH_WRITE_ITEM_COUNT, DEFAULT_DYNAMO_BATCH_WRITE_COMMAND_CONCURRENCY, getDynamoBatchWriteCommandConcurrency;
var init_constants = __esm({
  "node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/overrides/tagCache/constants.js"() {
    MAX_DYNAMO_BATCH_WRITE_ITEM_COUNT = 25;
    DEFAULT_DYNAMO_BATCH_WRITE_COMMAND_CONCURRENCY = 4;
    getDynamoBatchWriteCommandConcurrency = () => {
      ...
      const parsedDynamoBatchWriteCommandConcurrencyFromEnv = dynamoBatchWriteCommandConcurrencyFromEnv ? Number.parseInt(dynamoBatchWriteCommandConcurrencyFromEnv) : void 0;
      if (parsedDynamoBatchWriteCommandConcurrencyFromEnv && !Number.isNaN(parsedDynamoBatchWriteCommandConcurrencyFromEnv)) {
...
// node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/overrides/tagCache/dynamodb-lite.js
var dynamodb_lite_exports = {};
__export(dynamodb_lite_exports, {
  default: () => dynamodb_lite_default
});
import path from "node:path";
function buildDynamoKey(key) {
  const { NEXT_BUILD_ID } = process.env;
  return path.posix.join(NEXT_BUILD_ID ?? "", key);
}
function buildDynamoObject(path5, tags, revalidatedAt) {
  return {
    path: { S: buildDynamoKey(path5) },
    tag: { S: buildDynamoKey(tags) },
    revalidatedAt: { N: `${revalidatedAt ?? Date.now()}` }
  };
}
var awsClient, getAwsClient, awsFetch, tagCache, dynamodb_lite_default;
var init_dynamodb_lite = __esm({
  "node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/overrides/tagCache/dynamodb-lite.js"() {
    init_aws4fetch_esm();
    init_error();
    init_fetch();
    ...
    tagCache = {
      mode: "original",
      async getByPath(path5) {
        try {
          if (globalThis.openNextConfig.dangerous?.disableTagCache) {
            return [];
          }
          const { CACHE_DYNAMO_TABLE, NEXT_BUILD_ID } = process.env;
          const result = await awsFetch(JSON.stringify({
          ...
          debug("tags for path", path5, tags);
      ...
      async writeTags(tags) {
        try {
          ...
          for (const paramsChunk of toInsert) {
            await Promise.all(paramsChunk.map(async (params) => {
    ...
    dynamodb_lite_default = tagCache;
  }
});

// node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/overrides/queue/sqs-lite.js
var sqs_lite_exports = {};
__export(sqs_lite_exports, {
  default: () => sqs_lite_default
});
var awsClient2, getAwsClient2, awsFetch2, queue, sqs_lite_default;
var init_sqs_lite = __esm({
  "node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/overrides/queue/sqs-lite.js"() {
    init_aws4fetch_esm();
    init_error();
    init_fetch();
    ...
    queue = {
      send: async ({ MessageBody, MessageDeduplicationId, MessageGroupId }) => {
        try {
          ...
          const result = await awsFetch2(JSON.stringify({
    ...
    sqs_lite_default = queue;
  }
});

// node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/overrides/incrementalCache/s3-lite.js
var s3_lite_exports = {};
__export(s3_lite_exports, {
  default: () => s3_lite_default,
  getAwsClient: () => getAwsClient3
});
import path2 from "node:path";
function buildS3Key(key, extension) {
  const { CACHE_BUCKET_KEY_PREFIX, NEXT_BUILD_ID } = process.env;
  return path2.posix.join(CACHE_BUCKET_KEY_PREFIX ?? "", extension === "fetch" ? "__fetch" : "", NEXT_BUILD_ID ?? "", extension === "fetch" ? key : `${key}.${extension}`);
}
var awsClient3, getAwsClient3, awsFetch3, incrementalCache, s3_lite_default;
var init_s3_lite = __esm({
  "node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/overrides/incrementalCache/s3-lite.js"() {
    init_aws4fetch_esm();
    init_error();
    init_fetch();
    ...
    incrementalCache = {
      async get(key, cacheType) {
        const result = await awsFetch3(buildS3Key(key, cacheType ?? "cache"), {
          method: "GET"
        });
        ...
        const cacheData = await result.json();
        return {
          value: cacheData,
      ...
      async set(key, value, cacheType) {
        const response = await awsFetch3(buildS3Key(key, cacheType ?? "cache"), {
          method: "PUT",
      ...
      async delete(key) {
        const response = await awsFetch3(buildS3Key(key, "cache"), {
          method: "DELETE"
        });
    ...
    s3_lite_default = incrementalCache;
  }
});
...
async function edgeFunctionHandler(request) {
  const path5 = new URL(request.url).pathname;
  const routes = globalThis._ROUTES;
  const correspondingRoute = routes.find((route) => route.regex.some((r) => new RegExp(r).test(path5)));
...
async function resolveTagCache(tagCache2) {
  if (typeof tagCache2 === "function") {
    return tagCache2();
  }
  const m_1 = await Promise.resolve().then(() => (init_dynamodb_lite(), dynamodb_lite_exports));
  return m_1.default;
}
async function resolveQueue(queue2) {
  ...
  const m_1 = await Promise.resolve().then(() => (init_sqs_lite(), sqs_lite_exports));
  return m_1.default;
}
async function resolveIncrementalCache(incrementalCache2) {
  if (typeof incrementalCache2 === "function") {
    return incrementalCache2();
  }
  const m_1 = await Promise.resolve().then(() => (init_s3_lite(), s3_lite_exports));
  return m_1.default;
}
...
async function resolveProxyRequest(proxyRequest) {
  ...
  const m_1 = await Promise.resolve().then(() => (init_fetch2(), fetch_exports));
  return m_1.default;
}
...
// node_modules/.pnpm/@opennextjs+aws@3.6.5/node_modules/@opennextjs/aws/dist/core/routing/i18n/index.js
function isLocalizedPath(path5) {
  return NextConfig.i18n?.locales.includes(path5.split("/")[1].toLowerCase()) ?? false;
}
...
function constructNextUrl(baseUrl, path5) {
  const nextBasePath = NextConfig.basePath ?? "";
  const url = new URL(`${nextBasePath}${path5}`, baseUrl);
  return url.href;
}
...
async function computeCacheControl(path5, body, host, revalidate, lastModified) {
  let finalRevalidate = CACHE_ONE_YEAR;
  const existingRoute = Object.entries(PrerenderManifest.routes).find((p) => p[0] === path5)?.[1];
  ...
  const hash2 = (str) => createHash("md5").update(str).digest("hex");
  const etag = hash2(body);
  if (revalidate === 0) {
    return {
  ...
  if (finalRevalidate !== CACHE_ONE_YEAR) {
    ...
    if (isStale) {
      let url = NextConfig.trailingSlash ? `${path5}/` : path5;
      ...
      await globalThis.queue.send({
        ...
        MessageDeduplicationId: hash2(`${path5}-${lastModified}-${etag}`),
        MessageGroupId: generateMessageGroupId(path5)
      });
    }
    return {
...
function stringToRegexp(path5, keys, options) {
  return tokensToRegexp(parse2(path5, options), keys, options);
}
...
function routeMatcher(routeDefinitions) {
  ...
  return function matchRoute(path5) {
    const foundRoutes = regexp.filter((route) => route.regexp.test(path5));
...
function getNextConfigHeaders(event, configHeaders) {
  ...
  for (const { headers, has, missing, regex, source, locale } of configHeaders) {
    const path5 = locale === false ? event.rawPath : localizedRawPath;
    if (new RegExp(regex).test(path5) && checkHas(matcher, has) && checkHas(matcher, missing, true)) {
      const fromSource = match(source);
      const _match = fromSource(path5);
      headers.forEach((h) => {
        try {
...
function handleRewrites(event, rewrites) {
  ...
  const rewrite = rewrites.find((route) => {
    const path5 = route.locale === false ? rawPath : localizedRawPath;
    return new RegExp(route.regex).test(path5) && checkHas(matcher, route.has) && checkHas(matcher, route.missing, true);
  });
