...
function fixSchemaImports(filePath) {
  ...
  // 1. Check if schemaUtils import exists, if not add it
  ...
  calls.forEach(call => {
    ...
    if (expression.getText() === 'gen_random_uuid') {
      ...
      call.replaceWithText('schemaUtils.gen_random_uuid()');
      uuidReplacements++;
      changesMade = true;
    }
  });
  ...
  calls.forEach(call => {
    ...
    if (expression.getText() === 'generate_token') {
      // Replace the entire call expression
      call.replaceWithText('schemaUtils.generate_token');
      tokenReplacements++;
      changesMade = true;
    }
  });
  ...
  // But only if they're not already part of schemaUtils.users or other known patterns
  ...
  identifiers.forEach(identifier => {
    ...
    if (text === 'users') {
      ...
      if (!parentText.includes('schemaUtils.users') && 
          ...
          !parentText.includes('import { users')) {
...
module.exports = { fixSchemaImports }; 